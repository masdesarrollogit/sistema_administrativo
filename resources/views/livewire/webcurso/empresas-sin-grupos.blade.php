<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900 flex items-center gap-3">
                ⚠️ Empresas Sin Grupos Asociados
            </h1>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 mb-6 flex items-center justify-center gap-4">
            ...
            <div class="flex gap-2">
                <button 
                    ...
                    class="px-4 py-2 rounded-lg font-medium transition-all {{ $anioSeleccionado === $anioActual ? 'bg-amber-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                >
                    {{ $anioActual }}
                </button>
                <button 
                    wire:click="cambiarAnio({{ $anioAnterior }})"
                    class="px-4 py-2 rounded-lg font-medium transition-all {{ $anioSeleccionado === $anioAnterior ? 'bg-amber-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                >
                    {{ $anioAnterior }}
                </button>
            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
            <div class="bg-gradient-to-br from-amber-500 to-amber-700 rounded-xl p-4 text-white">
                <p class="text-amber-100 text-xs">EMPRESAS SIN GRUPOS ({{ $anioSeleccionado }})</p>
            ...
            <div class="bg-gradient-to-br from-blue-500 to-blue-700 rounded-xl p-4 text-white">
                <p class="text-blue-100 text-xs">CRÉDITO ASIGNADO</p>
            ...
            <div class="bg-gradient-to-br from-purple-500 to-purple-700 rounded-xl p-4 text-white">
                <p class="text-purple-100 text-xs">PROMEDIO EMPRESA</p>
                <p class="text-lg font-bold">{{ number_format($stats->promedio ?? 0, 0, ',', '.') }} €</p>
            </div>
        </div>

        {{-- Filtros --}}
        <div class="bg-white rounded-xl shadow-sm p-4 mb-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">🔍 Filtrar Empresas Sin Grupos ({{ $anioSeleccionado }})</h3>
            <div class="grid grid-cols-2 md:grid-cols-7 gap-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Población</label>
                    <input type="text" wire:model.live.debounce.300ms="filtroPoblacion" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-amber-500 focus:border-amber-500"
                           placeholder="Ciudad...">
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50/50">
                        <tr>
                            @foreach(config('webcurso.vistas.empresas_sin_grupos.columnas_tabla') as $columna => $label)
                                <th class="px-4 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ $label }}</th>
                            @endforeach
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-amber-50/30 transition-colors">
                                @foreach(config('webcurso.vistas.empresas_sin_grupos.columnas_tabla') as $columna => $label)
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        @if($columna === 'razon_social')
                                            <span class="font-black text-blue-700 hover:text-blue-900 cursor-pointer hover:underline decoration-2 underline-offset-4" 
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-center">
                                    <button wire:click="abrirModalEmpresa({{ $empresa->id }})"
                                            class="px-4 py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-amber-600 hover:text-white text-xs font-black transition-all uppercase tracking-tighter">
                                        Detalles
                                    </button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="{{ count(config('webcurso.vistas.empresas_sin_grupos.columnas_tabla')) + 1 }}" class="px-4 py-12 text-center text-gray-500">
                                    <div class="flex flex-col items-center gap-2">
                                        <span class="text-4xl">ℹ️</span>
                                        <p class="font-bold text-gray-400 uppercase text-xs tracking-widest">No hay empresas sin grupos asociados</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    {{-- Modal Compartido --}}
    @include('livewire.webcurso.partials.modal-empresa', ['viewName' => 'empresas_sin_grupos'])
</div>
