...
| **Particular** | Persona que paga el curso de su bolsillo. No hay bonificacion FUNDAE, asi que no lleva grupo formativo: se matricula individualmente en Moodle. Se modela como `MatriculaAutonoma` con `modalidad='particular'`. No tiene empresa cliente; la empresa donde trabaja se guarda como texto libre en `alumnos.empresa_texto`, solo para control interno. |
| **Matricula individual** | Matricula en Moodle sin grupo formativo FUNDAE. Dos modalidades: **autonomo 2x1** (curso gratis que acompana a un bonificado) y **particular** (de pago). Ninguna genera XML, PDF de notificacion ni id_grupo_fundae; ambas SI entran en Reportes Moodle. |
| **Grupo externo** | Grupo formativo cuya bonificacion FUNDAE la gestiona la propia empresa cliente (candidato `empresa_externa`). Esa empresa NO se registra en `empresas` —no calculamos su credito—, solo en `empresas_externas` para control. Ellos comunican accion y grupo en SU aplicativo FUNDAE y nos trasladan el codigo resultante como texto libre (ej. `241/3`); nosotros solo impartimos el curso en Moodle. Se modela como `GrupoFormativo` con `gestion_externa = true`. No genera ID de grupo nuestro, ni XML de inicio, ni PDF de notificacion. |
